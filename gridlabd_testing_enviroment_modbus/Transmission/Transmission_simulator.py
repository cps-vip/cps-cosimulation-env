...
if __name__ == "__main__":
    ...
    for t in range(0, total_inteval, pf_interval):
        ...
        packed_voltage_real = struct.pack('>f', voltage_gld.real)  # '>f' for big-endian 32-bit float
        reg1, reg2 = struct.unpack('>HH', packed_voltage_real)  # '>HH' for two big-endian 16-bit parts

        packed_voltage_imag = struct.pack('>f', voltage_gld.imag)  # '>f' for big-endian 32-bit float
        reg3, reg4 = struct.unpack('>HH', packed_voltage_imag)  # '>HH' for two big-endian 16-bit parts

        client.write_registers(address, [reg1, reg2, reg3, reg4])
    ...
    numpy.savetxt(
        "../results/Transmission_Voltages.csv",
    ...
    numpy.savetxt(
        "../results/Transmission_MW_demand.csv",
