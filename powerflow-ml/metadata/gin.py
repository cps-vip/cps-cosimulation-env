...
class PowerFlowGIN(torch.nn.Module):
    def __init__(self, metadata_dim):
        ...
        self.conv1 = GINConv(torch.nn.Sequential(torch.nn.Linear(1 + metadata_dim, 16), torch.nn.ReLU(), torch.nn.Linear(16, 16)))
    ...
    def forward(self, x, edge_index, metadata):
        ...
        # Pass through first GIN layer
        x = self.conv1(x, edge_index)
        x = F.relu(x)
        
        # Pass through second GIN layer
...
if (__name__ == "__main__"):
    ...
    # Instantiate the GIN model with metadata
    gin_model = PowerFlowGIN(metadata_dim)
    ...
    for _ in range(num_iterations):
        # Forward pass through the GIN model with metadata
        x = gin_model(x, edge_index, metadata)
        
        # Clamp voltage magnitudes to ensure they remain positive
        x = torch.relu(x)

    # Display the final voltage magnitudes
    print("Final Voltage Magnitudes (GIN with Metadata):")
    print(x)
