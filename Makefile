...
clean:: $1-clean
 ...
 $1-clean-config:
	find $2/ -maxdepth 1 -mindepth 1 -type d -name build -exec rm -rf {} \; && \
 	find $2/ -maxdepth 1 -mindepth 1 -type d -name install -exec rm -rf {} \;
clean-config:: $1-clean-config
endef

$(eval $(call create-cmake-targets,helics,HELICS,-DCMAKE_INSTALL_PREFIX=$(REPO_ROOT)/install -DHELICS_BUILD_CXX_SHARED_LIB=ON -DHELICS_BUILD_EXAMPLES=ON -DHELICS_BUILD_TESTS=ON))
