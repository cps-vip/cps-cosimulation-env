...
if __name__ == "__main__":
    ...
    logger.info("Got to point A")
    for t in range(0, total_inteval, sensing_interval):

        ############################ Custom ?????? Publishing Voltage to GridLAB-D #######################################################

        logger.info("Got to point B")
        for i in range(0, pubkeys_count):
            pub = pubid["m{}".format(i)]
            logger.info("{}/{}: Publishing Switch State: {}".format(federate_name, h.helicsPublicationGetName(pub), "OPEN"))
            status = h.helicsPublicationPublishString(pub, "OPEN")
        ...
        sub = subid["m{}".format(subkeys_count - 1)]
        name = h.helicsInputGetTarget(sub) 
        current_state = h.helicsInputGetString(sub)
        logger.info("{}: Substation {} to Distribution System = {}".format(federate_name, name, current_state))
